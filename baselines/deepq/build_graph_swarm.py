...
def set_cover(subsets, agent_ph):
    ...
    cut = tf.transpose(subsets, [1, 0, 2])[agent_ph]
...
#
#     def body(cover):
#         universe_extended = tf.reshape(tf.tile(universe - cover, [1, N]), [B, N, M])
#         s = tf.reduce_sum(tf.multiply(subsets, universe_extended), axis=1)
#         c = tf.one_hot(tf.argmax(s, axis=1), M, dtype=tf.int32)
#         z = tf.zeros([B, M], tf.int32)
#         c = tf.where(tf.reshape(tf.tile(tf.reduce_max(s, axis=1) > 0, [M]), [B, M]), c, z)
#         return cover + c
#
#     def cond(cover):
#         return tf.reduce_all(tf.not_equal(cover, universe))
#
#     cover = tf.zeros([B, M], tf.int32)
#
#     universe = tf.where(tf.reduce_sum(subsets, axis=1) > 0,
#                         tf.ones([B, M], tf.int32),
#                         tf.zeros([B, M], tf.int32))
#
#     loop = tf.while_loop(cond, body, [cover])
#
#     cut = tf.transpose(subsets, [1, 0, 2])[agent_ph]
...
`       See the top of the file for details.
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for clone in range(num_clones):
            with tf.variable_scope("clone{}".format(clone), reuse=reuse):
                q_values_online = q_func(observations_ph.get(), num_actions, scope="q_func")
                q_values_target = q_func(observations_ph.get(), num_actions, scope="target_q_func")

                policy_action = tf.argmax(q_values_target, axis=1)

                q_value_threshold = tf.reduce_sum(q_values_online * tf.one_hot(policy_action, num_actions), 1)

                clone_action_set = tf.where((q_values_online - q_value_threshold) >= 0,
                                            tf.ones([tf.shape(observations_ph.get())[0], num_actions], tf.int32),
                                            tf.zeros([tf.shape(observations_ph.get())[0], num_actions], tf.int32))

                clone_action_sets.append(clone_action_set)
        clone_actions = tf.stack(clone_action_sets, axis=1)

        deterministic_actions = set_cover(clone_actions, agent_ph)

        indexes = tf.stack([tf.range(0, tf.shape(deterministic_actions)[0], 1), deterministic_actions], axis=1)
...
def build_train(make_obs_ph, q_func, num_actions, optimizer, num_clones, grad_norm_clipping=None, gamma=1.0,
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        for clone in range(num_clones):
            ...
            with tf.variable_scope(clone_scope, reuse=reuse):
                ...
                select_optimizer.append(optimize_expr)
                # select_optimizer.append(tf.cond(tf.equal(shared_agents_ph[clone], 1), true_fn=lambda: optimize_expr, false_fn=tf.no_op))
